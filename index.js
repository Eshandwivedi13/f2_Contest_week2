...
function handle(event){
    ...
    if(val.toLowerCase() === "increment"){
        ...
        if(count == 0){
            ...
            count--;
            if(count <=0)  clearButton.style.display = "none";//count 0, nothing to clear
            countingElem.innerHTML = `Your Current Count is : ${count}`;//use innerText insteadgi
        }
    }else{
        clearButton.style.display = "none";//or elem.style.cssText = `all styles here`
        count = 0;
        countingElem.innerText = `Your Current Count is : ${count}`;
    }
